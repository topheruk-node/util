...
@customElement("lit-range-slider")
export class LitRangeSliderElement extends LitElement {
    @property() color = 'red';
    pressed = false;
    ...
    render() {
        ...
        <button 
          ...
          @pointerup=${this.up}
          @keydown=${(e: KeyboardEvent) => e.key === " " && this.down()}
        ...
        >
          <svg 
            @pointerdown=${() => this.dispatchEvent(new Event('button-press'))}
            @pointerup=${() => this.dispatchEvent(new Event('button-release'))}
            width="18mm" 
            height="12mm" 
            version="1.1" 
            viewBox="-3 0 18 12"     
...
declare global {
    interface HTMLElementTagNameMap {
        "lit-range-slider": LitRangeSliderElement;
    }
}